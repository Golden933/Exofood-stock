...
function financeGraph($history,$symbol='€'){
 ...
 if(!$vals)return '<p class="muted">Pas encore assez de données pour afficher l’évolution.</p>';

 // On garde au maximum 8 points pour que le graphique reste immédiatement lisible.
 if(count($history)>8){$history=array_slice($history,-8);$vals=array_map('floatval',array_column($history,'value'));}
 $n=count($vals);$min=min($vals);$max=max($vals);
 $pad=max(10,($max-$min)*0.18);
 ...
 $html.='<div><span class="muted">Évolution</span><strong>'.$trend.' · '.$sign.number_format($deltaPct,1,',',' ').'%</strong></div>';
 $html.='</div>';
 $html.='<div class="muted" style="margin:4px 0 12px">Chaque point représente la valeur totale du stock après une modification ou un mouvement.</div>';
 $html.='<div style="overflow-x:auto"><svg class="finance-svg simple" viewBox="0 0 800 260" role="img" aria-label="Évolution simple de la valeur totale du stock">';

 // 3 repères horizontaux seulement.
 ...
 foreach($pts as $i=>$p){
   ...
   // valeur visible au-dessus de chaque point
 ...
 $html.='</svg></div>';
 $html.='<div class="finance-legend"><span><b>Début :</b> '.$fmt($first).'</span><span><b>Actuel :</b> '.$fmt($last).'</span><span><b>Écart :</b> '.$sign.$fmt($delta).'</span></div>';
 $html.='</div>';
 return $html;
}
