...
<?php $vals=array_column($valueHistory,'value');$mx=max(max($vals),1);$mn=min($vals);?><div class="chart"><?php foreach($valueHistory as $vh):?><div class="col" style="height:<?=max(5,round(((float)$vh['value']/$mx)*100))?>%" data-v="<?=number_format((float)$vh['value'],2,',',' ')?> <?=$currencySymbol?>"></div><?php endforeach?></div>
...
<div class="card"><h3>Indicateurs financiers</h3><p>Valeur actuelle : <b><?=number_format($totalValue,2,',',' ')?> <?=$currencySymbol?></b></p><p>Valeur critique : <b><?=number_format($criticalValue,2,',',' ')?> <?=$currencySymbol?></b></p><p>Variation : <b><?=$valueChange>=0?'+':''?><?=number_format($valueChange,2,',',' ')?> <?=$currencySymbol?></b></p><p>Réapprovisionnement cible : <b><?=esc($settings['default_restock_hours']??'4.5')?> h</b></p></div>
</div>
