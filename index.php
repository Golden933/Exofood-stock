...
if ($_SERVER['REQUEST_METHOD']==='POST') {
 ...
 if ($action==='settings_update') {
   ...
   $st->execute(['require_movement_reason',isset($_POST['require_movement_reason'])?'1':'0']);
   $st->execute(['finance_dashboard_enabled',isset($_POST['finance_dashboard_enabled'])?'1':'0']);
   $st->execute(['compact_tables',isset($_POST['compact_tables'])?'1':'0']);
   $st->execute(['confirm_sensitive_actions',isset($_POST['confirm_sensitive_actions'])?'1':'0']);
 }
 if ($action==='movement') {
   $reason=$reason;
