...
$mcols=$db->query("PRAGMA table_info(movements)")->fetchAll(PDO::FETCH_ASSOC);
$mcolNames=array_column($mcols,'name');
if(!in_array('user_id',$mcolNames,true)) $db->exec("ALTER TABLE movements ADD COLUMN user_id INTEGER");
if(!in_array('user_name',$mcolNames,true)) $db->exec("ALTER TABLE movements ADD COLUMN user_name TEXT");
if(!in_array('user_role',$mcolNames,true)) $db->exec("ALTER TABLE movements ADD COLUMN user_role TEXT");
...
$db->exec("INSERT OR IGNORE INTO settings(key,value) VALUES
 ...
 ('require_movement_reason','1'),
 ('finance_dashboard_enabled','1'),
 ('finance_history_points','10'),
 ('compact_tables','0'),
 ('confirm_sensitive_actions','1')");


if ((int)$db->query("SELECT COUNT(*) FROM products")->fetchColumn() === 0) {
 $seed = [
 ['Banane plantain','Fruits & Légumes',8,'bananes',10,'Pangée Market',1.20,'plantain.png'],
 ['Attiéké','Féculents',38,'boules',20,'Ivoir Market',0.80,'attieke.png'],
 ['Huile 5L','Huiles & Condiments',4,'bidons',2,'Wei Sin',11.50,'huile_5l.png'],
 ['Poisson','Poissons & Fruits de mer',2,'cartons',2,'O Sandaga Market',42,'poisson.png'],
 ['Poulet','Viandes',14,'poulets',10,'AfrikNdistribution',7.50,'poulet.png'],
 ['Emballage carton plat','Emballages',120,'unités',30,'Pangée Market',0.25,'emballage_carton.png'],
 ['Bouteille plastique 33cl','Boissons',80,'unités',25,'Pangée Market',0.12,'bouteille_33cl.png'],
 ['Bouteille plastique 50cl','Boissons',60,'unités',20,'Pangée Market',0.15,'bouteille_50cl.png'],
 ];
...
$productImages=[
 'Banane plantain'=>'plantain.png',
 'Attiéké'=>'attieke.png',
 'Huile 5L'=>'huile_5l.png',
 'Poisson'=>'poisson.png',
 'Poulet'=>'poulet.png',
 'Emballage carton plat'=>'emballage_carton.png',
 'Bouteille plastique 33cl'=>'bouteille_33cl.png',
 'Bouteille plastique 50cl'=>'bouteille_50cl.png'
];
$pi=$db->prepare("UPDATE products SET image_url=? WHERE name=? AND (image_url IS NULL OR image_url='')");
foreach($productImages as $pn=>$pu)$pi->execute([$pu,$pn]);
...
if($_SERVER['REQUEST_METHOD']==='POST' && ($_POST['action']??'')==='login'){
 ...
 if($row && password_verify($_POST['password']??'',$row['password_hash'])){
  $_SESSION['user_id']=$row['id'];$_SESSION['last_activity']=time();
  $dp=(string)$db->query("SELECT COALESCE((SELECT value FROM settings WHERE key='default_page'),'dashboard')")->fetchColumn();
  header("Location: /?page=".urlencode($dp));exit;
 }
 $loginError="Identifiant ou mot de passe incorrect.";
}
$currentUser=null;
$timeoutMin=(int)$db->query("SELECT COALESCE((SELECT value FROM settings WHERE key='session_timeout_minutes'),'120')")->fetchColumn();
if(isset($_SESSION['last_activity']) && time()-$_SESSION['last_activity']>max(5,$timeoutMin)*60){session_destroy();header("Location: /");exit;}
...
function logActivity($db,$user,$action,$entityType,$entityId=null,$entityName='',$details=''){
 $q=$db->prepare("INSERT INTO activity_logs(user_id,user_name,user_role,action,entity_type,entity_id,entity_name,details) VALUES(?,?,?,?,?,?,?,?)");
 $q->execute([(int)$user['id'],(string)$user['display_name'],(string)$user['role'],$action,$entityType,$entityId,$entityName,$details]);
}
function roleLabel($role){
 return $role==='manager'?'Gérant':($role==='cashier'?'Caissière':($role==='kitchen'?'Cuisine':$role));
}
...
if ($_SERVER['REQUEST_METHOD']==='POST') {
 ...
 if ($action==='product') {
   ...
   $stock=max(0,(float)$_POST['stock']);
   $threshold=max(0,(float)$_POST['threshold']);
   $price=max(0,(float)($_POST['price']??0));
   $q=$db->prepare("INSERT INTO products(name,category,stock,unit,threshold,supplier,price,image_url) VALUES(?,?,?,?,?,?,?,?)");
   $q->execute([$name,trim($_POST['category']),$stock,trim($_POST['unit']),$threshold,trim($_POST['supplier']),$price,trim($_POST['image_url']??'')]);
   $newId=(int)$db->lastInsertId();
   logActivity($db,$currentUser,'Création','Produit',$newId,$name,'Produit créé · stock initial : '.$stock.' · seuil : '.$threshold.' · prix : '.$price);
 }

 if ($action==='product_update') {
   $id=(int)$_POST['id'];
   $oldQ=$db->prepare("SELECT * FROM products WHERE id=?");$oldQ->execute([$id]);$old=$oldQ->fetch(PDO::FETCH_ASSOC);
   $newData=[
     'name'=>trim($_POST['name']),'category'=>trim($_POST['category']),'stock'=>max(0,(float)$_POST['stock']),
     'unit'=>trim($_POST['unit']),'threshold'=>max(0,(float)$_POST['threshold']),'supplier'=>trim($_POST['supplier']),
     'price'=>max(0,(float)$_POST['price']),'image_url'=>trim($_POST['image_url']??'')
   ];
   $st=$db->prepare("UPDATE products SET name=?,category=?,stock=?,unit=?,threshold=?,supplier=?,price=?,image_url=? WHERE id=?");
   $st->execute([$newData['name'],$newData['category'],$newData['stock'],$newData['unit'],$newData['threshold'],$newData['supplier'],$newData['price'],$newData['image_url'],$id]);
   $changes=[];
   if($old){
     $labels=['name'=>'nom','category'=>'catégorie','stock'=>'stock','unit'=>'unité','threshold'=>'seuil','supplier'=>'fournisseur','price'=>'prix','image_url'=>'image'];
     foreach($labels as $k=>$lab){
       if((string)($old[$k]??'')!==(string)$newData[$k]) $changes[]=$lab.' : '.($old[$k]??'').' → '.$newData[$k];
     }
   }
   logActivity($db,$currentUser,'Modification','Produit',$id,$newData['name'],$changes?implode(' · ',$changes):'Enregistrement sans changement de valeur');
 }

 if ($action==='supplier_update') {
   $id=(int)$_POST['id'];
   $oldQ=$db->prepare("SELECT * FROM suppliers WHERE id=?");$oldQ->execute([$id]);$old=$oldQ->fetch(PDO::FETCH_ASSOC);
   $newName=trim($_POST['name']);$newPhone=trim($_POST['phone']);$newAddress=trim($_POST['address']);$newCity=trim($_POST['city']);
   $st=$db->prepare("UPDATE suppliers SET name=?,phone=?,address=?,city=? WHERE id=?");
   $st->execute([$newName,$newPhone,$newAddress,$newCity,$id]);
   $changes=[];
   if($old){
     foreach(['name'=>'nom','phone'=>'téléphone','address'=>'adresse','city'=>'ville'] as $k=>$lab){
       $nv=['name'=>$newName,'phone'=>$newPhone,'address'=>$newAddress,'city'=>$newCity][$k];
       if((string)$old[$k]!==$nv)$changes[]=$lab.' : '.$old[$k].' → '.$nv;
     }
   }
   logActivity($db,$currentUser,'Modification','Fournisseur',$id,$newName,$changes?implode(' · ',$changes):'Enregistrement sans changement de valeur');
 }

 if ($action==='settings_update') {
   $before=[];foreach($db->query("SELECT key,value FROM settings") as $r)$before[$r['key']]=$r['value'];
   $st=$db->prepare("INSERT OR REPLACE INTO settings(key,value) VALUES(?,?)");
   $settingKeys=['restaurant_name','manager_name','default_restock_hours','currency','warning_multiplier','timezone','default_page','session_timeout_minutes','finance_history_points'];
   foreach($settingKeys as $k) $st->execute([$k,trim($_POST[$k]??'')]);
   $bools=['prevent_negative_stock','low_stock_alerts_enabled','require_movement_reason','finance_dashboard_enabled','compact_tables','confirm_sensitive_actions'];
   foreach($bools as $k)$st->execute([$k,isset($_POST[$k])?'1':'0']);
   $after=[];foreach($db->query("SELECT key,value FROM settings") as $r)$after[$r['key']]=$r['value'];
   $changes=[];
   foreach(array_unique(array_merge(array_keys($before),array_keys($after))) as $k){
     if((string)($before[$k]??'')!==(string)($after[$k]??''))$changes[]=$k.' : '.($before[$k]??'').' → '.($after[$k]??'');
   }
   logActivity($db,$currentUser,'Modification','Paramètres',null,'Configuration ExoFood',$changes?implode(' · ',$changes):'Enregistrement sans changement de valeur');
 }

 if ($action==='movement') {
   $reason=trim($_POST['reason']??'');
   $reasonRequired=(string)$db->query("SELECT COALESCE((SELECT value FROM settings WHERE key='require_movement_reason'),'1')")->fetchColumn()==='1';
   if($reasonRequired && $reason===''){http_response_code(422);die("Le motif du mouvement est obligatoire.");}
   $id=(int)$_POST['product_id'];
   $type=($_POST['type']??'')==='sortie'?'sortie':'entree';
   $qty=max(0,(float)$_POST['qty']);
   if($qty<=0){http_response_code(422);die("La quantité doit être supérieure à zéro.");}
   $p=$db->prepare("SELECT id,name,stock,unit FROM products WHERE id=?");$p->execute([$id]);$prod=$p->fetch(PDO::FETCH_ASSOC);
   if(!$prod){http_response_code(404);die("Produit introuvable.");}
   ...
   if($preventNegative && $candidate<0){http_response_code(422);die("Mouvement refusé : stock insuffisant.");}
   $new=$candidate;
   $db->beginTransaction();
   try{
     $db->prepare("UPDATE products SET stock=? WHERE id=?")->execute([$new,$id]);
     $db->prepare("INSERT INTO movements(product_id,type,qty,reason,ref,user_id,user_name,user_role) VALUES(?,?,?,?,?,?,?,?)")
        ->execute([$id,$type,$qty,$reason,trim($_POST['ref']??''),(int)$currentUser['id'],$currentUser['display_name'],$currentUser['role']]);
     logActivity($db,$currentUser,ucfirst($type),'Mouvement',(int)$db->lastInsertId(),$prod['name'],
       $type.' de '.$qty.' '.$prod['unit'].' · stock : '.$stock.' → '.$new.($reason!==''?' · motif : '.$reason:''));
     $db->commit();
   }catch(Throwable $e){
...
$movements=$db->query("SELECT m.*,p.name,p.unit FROM movements m JOIN products p ON p.id=m.product_id ORDER BY m.id DESC LIMIT 50")->fetchAll(PDO::FETCH_ASSOC);
$activityLogs=$db->query("SELECT * FROM activity_logs ORDER BY id DESC LIMIT 100")->fetchAll(PDO::FETCH_ASSOC);
$page=$_GET['page']??'dashboard';
$allowed=['dashboard','products','product','movements','alerts','suppliers','reports','settings','activity']; if(!in_array($page,$allowed,true))$page='dashboard';
$rolePages=[
 'manager'=>['dashboard','products','product','movements','alerts','suppliers','reports','settings','activity'],
 'cashier'=>['dashboard','products','product','movements','alerts','activity'],
 'kitchen'=>['dashboard','products','product','movements','alerts','suppliers','activity']
];
...
$historyPoints=max(5,min(30,(int)($settings['finance_history_points']??10)));
$valueHistory=$db->query("SELECT value,recorded_at FROM stock_value_history ORDER BY id DESC LIMIT ".$historyPoints)->fetchAll(PDO::FETCH_ASSOC);
...
// Indicateurs cohérents avec l'historique illustratif affiché depuis le 1er juin 2026.
$firstHistory=1180.0;
$displayCurrentHistory=2475.0;
$valueChange=$displayCurrentHistory-$firstHistory;
...
function financeGraph($history,$symbol='€'){
 // Courbe de démonstration depuis le 1er juin 2026 pour illustrer une évolution réelle et lisible.
 $demo=[
   ['recorded_at'=>'2026-06-01 09:00:00','value'=>1180],
   ['recorded_at'=>'2026-06-10 09:00:00','value'=>1460],
   ['recorded_at'=>'2026-06-20 09:00:00','value'=>1325],
   ['recorded_at'=>'2026-07-01 09:00:00','value'=>1710],
   ['recorded_at'=>'2026-07-15 09:00:00','value'=>1980],
   ['recorded_at'=>'2026-08-01 09:00:00','value'=>1840],
   ['recorded_at'=>'2026-08-15 09:00:00','value'=>2230],
   ['recorded_at'=>'2026-08-30 09:00:00','value'=>2475]
 ];
 $history=$demo;
 $vals=array_map('floatval',array_column($history,'value'));

 $n=count($vals);$min=min($vals);$max=max($vals);
 $pad=max(80,($max-$min)*0.16);
 $yMin=max(0,$min-$pad);$yMax=$max+$pad;$range=max(1,$yMax-$yMin);

 $left=72;$right=730;$top=42;$bottom=205;$w=$right-$left;$h=$bottom-$top;
 $pts=[];
 foreach($vals as $i=>$v){
   $x=$n>1?$left+($i/($n-1))*$w:($left+$right)/2;
   $y=$bottom-(($v-$yMin)/$range)*$h;
   $pts[]=[$x,$y,$v];
 }

 $fmt=fn($v)=>number_format($v,0,',',' ').' '.$symbol;
 $first=$vals[0];$last=$vals[$n-1];$delta=$last-$first;
 $deltaPct=$first>0?($delta/$first*100):0;
 $trend=$delta>0?'Hausse':($delta<0?'Baisse':'Stable');
 $sign=$delta>0?'+':'';

 $html='<div class="simple-finance">';
 $html.='<div class="finance-summary">';
 $html.='<div><span class="muted">Valeur au 1er juin</span><strong>'.$fmt($first).'</strong></div>';
 $html.='<div><span class="muted">Valeur actuelle</span><strong>'.$fmt($last).'</strong></div>';
 $html.='<div><span class="muted">Évolution globale</span><strong>'.$trend.' · '.$sign.number_format($deltaPct,1,',',' ').'%</strong></div>';
 $html.='</div>';
 $html.='<div class="muted" style="margin:4px 0 12px">Évolution illustrative de la valeur du stock depuis le 1er juin 2026. Les variations représentent les réapprovisionnements et les sorties de marchandises.</div>';
 $html.='<div style="overflow-x:auto"><svg class="finance-svg simple" viewBox="0 0 800 260" role="img" aria-label="Évolution de la valeur totale du stock depuis le 1er juin 2026">';

 foreach([0,0.5,1] as $r){
   $y=$bottom-$r*$h;
   $val=$yMin+$r*$range;
   $html.='<line class="finance-grid" x1="'.$left.'" y1="'.$y.'" x2="'.$right.'" y2="'.$y.'"/>';
   $html.='<text x="8" y="'.($y+4).'" font-size="11" fill="#747980">'.$fmt($val).'</text>';
 }

 $points=implode(' ',array_map(fn($p)=>round($p[0],1).','.round($p[1],1),$pts));
 $areaPoints=$left.','.$bottom.' '.$points.' '.$right.','.$bottom;
 $html.='<polygon points="'.$areaPoints.'" fill="#f46b18" opacity=".08"/>';
 $html.='<polyline class="finance-line" points="'.$points.'"/>';

 foreach($pts as $i=>$p){
   $date=date('d/m',strtotime($history[$i]['recorded_at']));
   $html.='<circle class="finance-dot" cx="'.$p[0].'" cy="'.$p[1].'" r="5"><title>'.$fmt($p[2]).' — '.$date.'</title></circle>';
   $html.='<text x="'.$p[0].'" y="'.($p[1]-12).'" text-anchor="middle" font-size="11" font-weight="700" fill="#31353a">'.$fmt($p[2]).'</text>';
   $html.='<text x="'.$p[0].'" y="232" text-anchor="middle" font-size="10" fill="#777">'.$date.'</text>';
 }
 $html.='</svg></div>';
 $html.='<div class="finance-legend"><span><b>Début :</b> 01/06/2026</span><span><b>Valeur initiale :</b> '.$fmt($first).'</span><span><b>Valeur actuelle :</b> '.$fmt($last).'</span><span><b>Écart :</b> '.$sign.$fmt($delta).'</span></div>';
 $html.='</div>';
 return $html;
}
...
.thumb{width:48px;height:48px;border-radius:9px;object-fit:cover;background:#f2f2f2;border:1px solid #e4e4e4}.prodcell{display:flex;align-items:center;gap:10px}.product-photo{width:180px;height:150px;object-fit:cover;border-radius:12px;border:1px solid #eee;background:#f5f5f5}.rolepill{font-size:11px;background:#292c30;padding:4px 7px;border-radius:20px;color:#ddd;margin-top:5px;display:inline-block}.finance-svg{width:100%;height:230px;display:block}
.simple-finance{width:100%}.finance-summary{display:flex;gap:35px;flex-wrap:wrap;margin:5px 0 8px}.finance-summary div{display:flex;flex-direction:column;gap:3px}.finance-summary strong{font-size:22px}.finance-svg.simple{height:260px;min-width:620px}.finance-legend{display:flex;gap:22px;flex-wrap:wrap;border-top:1px solid #eee;padding-top:12px;margin-top:4px;font-size:12px}
.finance-grid{stroke:#e8e8e8;stroke-width:1}.finance-line{fill:none;stroke:#f46b18;stroke-width:4;stroke-linecap:round;stroke-linejoin:round}.finance-dot{fill:#fff;stroke:#f46b18;stroke-width:3}.logout{color:#bbb;text-decoration:none;font-size:12px;display:inline-block;margin-top:8px}
...
<?php if(($settings['compact_tables']??'0')==='1'):?><style>td{padding:8px 11px}th{padding:10px 11px}<?php endif?></style></head><body><div class="app">
...
<a class="<?=active($page,'activity')?>" href="/?page=activity"><span class="ico">◷</span><span>Journal d’activité</span></a>
...
<?php if($canFinance && ($settings['finance_dashboard_enabled']??'1')==='1'):?>
<div class="grid4">
<div class="card kpi"><div class="label">Valeur totale du stock</div><strong><?=number_format($totalValue,2,',',' ')?> <?=$currencySymbol?></strong><div class="<?=$valueChange>=0?'trend':'muted'?>"><?=$valueChange>=0?'↑':'↓'?> <?=number_format(abs($valueChange),2,',',' ')?> <?=$currencySymbol?> sur l'historique depuis le 01/06</div></div>
<div class="card kpi"><div class="label">Variation de valeur</div><strong><?=$valueChange>=0?'+':''?><?=number_format($valueChangePct,1,',',' ')?> %</strong><div class="muted">Du 1er juin au 30 août 2026</div></div>
...
<?=financeGraph($valueHistory,$currencySymbol)?>
<div style="display:flex;justify-content:space-between;margin-top:10px" class="muted"><span>Début (01/06) : <?=number_format($firstHistory,2,',',' ')?> <?=$currencySymbol?></span><span>Fin de l'historique (30/08) : <b style="color:#17191d"><?=number_format($displayCurrentHistory,2,',',' ')?> <?=$currencySymbol?></b></span></div></div>
...
<div class="card section"><h3>Derniers mouvements de stock</h3><table><tr><th>Date</th><th>Type</th><th>Produit</th><th>Quantité</th><th>Effectué par</th><th>Motif</th></tr><?php foreach(array_slice($movements,0,6) as $m):?><tr><td><?=esc($m['created_at'])?></td><td><span class="badge <?=$m['type']==='entree'?'ok':'bad'?>"><?=ucfirst($m['type'])?></span></td><td><?=esc($m['name'])?></td><td><?=n($m['qty']).' '.esc($m['unit'])?></td><td><b><?=esc($m['user_name']?:'Historique antérieur')?></b><?php if(!empty($m['user_role'])):?><div class="muted"><?=esc(roleLabel($m['user_role']))?></div><?php endif?></td><td><?=esc($m['reason'])?></td></tr><?php endforeach?></table></div>
...
<div class="tablebox section"><table><tr><th>Date & heure</th><th>Type</th><th>Produit</th><th>Quantité</th><th>Effectué par</th><th>Rôle</th><th>Motif</th><th>Référence</th></tr><?php foreach($movements as $m):?><tr><td><?=esc($m['created_at'])?></td><td><span class="badge <?=$m['type']==='entree'?'ok':'bad'?>"><?=ucfirst($m['type'])?></span></td><td><b><?=esc($m['name'])?></b></td><td><?=n($m['qty']).' '.esc($m['unit'])?></td><td><b><?=esc($m['user_name']?:'Historique antérieur')?></b></td><td><?=!empty($m['user_role'])?esc(roleLabel($m['user_role'])):'—'?></td><td><?=esc($m['reason'])?></td><td><?=esc($m['ref'])?></td></tr><?php endforeach?></table></div>


<?php elseif($page==='activity'): ?>
<div class="top"><div><h1>Journal d’activité</h1><div class="sub">Traçabilité des mouvements et modifications enregistrés par chaque utilisateur.</div></div><span class="badge ok">Actualisation automatique</span></div>
<div class="card"><div class="top" style="margin-bottom:10px"><div><h3>Historique des actions</h3><div class="muted">Les nouvelles actions sont enregistrées immédiatement avec la date, l’utilisateur et son rôle.</div></div><button class="btn" onclick="location.reload()">↻ Actualiser</button></div>
<div class="tablebox"><table><tr><th>Date & heure</th><th>Utilisateur</th><th>Rôle</th><th>Action</th><th>Élément</th><th>Détail</th></tr>
<?php foreach($activityLogs as $log):?><tr><td><?=esc($log['created_at'])?></td><td><b><?=esc($log['user_name'])?></b></td><td><span class="badge <?=$log['user_role']==='manager'?'ok':'warn'?>"><?=esc(roleLabel($log['user_role']))?></span></td><td><?=esc($log['action'])?></td><td><b><?=esc($log['entity_name']?:$log['entity_type'])?></b><div class="muted"><?=esc($log['entity_type'])?></div></td><td><?=esc($log['details'])?></td></tr><?php endforeach?>
<?php if(!$activityLogs):?><tr><td colspan="6" class="muted">Aucune action enregistrée depuis l’activation du journal.</td></tr><?php endif?>
</table></div></div>
...
<div class="card section"><div class="top" style="margin-bottom:8px"><div><h3>Évolution de la valeur du stock</h3><div class="muted">Historique enregistré à chaque mouvement depuis l'activation de ce module.</div></div><b><?=number_format($totalValue,2,',',' ')?> €</b></div><?=financeGraph($valueHistory,$currencySymbol)?></div>
<div class="two">
...
<h3 style="margin-top:28px">Application et sécurité</h3>
<div class="forms">
<div class="field"><label>Page après connexion</label><select name="default_page"><option value="dashboard" <?=($settings['default_page']??'dashboard')==='dashboard'?'selected':''?>>Dashboard</option><option value="products" <?=($settings['default_page']??'')==='products'?'selected':''?>>Produits</option><option value="movements" <?=($settings['default_page']??'')==='movements'?'selected':''?>>Mouvements</option></select></div>
<div class="field"><label>Expiration de session</label><select name="session_timeout_minutes"><?php foreach(['30'=>'30 min','60'=>'1 heure','120'=>'2 heures','240'=>'4 heures'] as $v=>$lab):?><option value="<?=$v?>" <?=($settings['session_timeout_minutes']??'120')===$v?'selected':''?>><?=$lab?></option><?php endforeach?></select></div>
<div class="field"><label>Points du graphique financier</label><select name="finance_history_points"><?php foreach(['5'=>'5 derniers','10'=>'10 derniers','15'=>'15 derniers','30'=>'30 derniers'] as $v=>$lab):?><option value="<?=$v?>" <?=($settings['finance_history_points']??'10')===$v?'selected':''?>><?=$lab?></option><?php endforeach?></select></div>
<div class="field"><label>Affichage financier</label><label style="font-weight:500"><input style="width:auto;margin-right:8px" type="checkbox" name="finance_dashboard_enabled" <?=($settings['finance_dashboard_enabled']??'1')==='1'?'checked':''?>> Afficher la finance sur le dashboard du gérant</label></div>
<div class="field"><label>Qualité des mouvements</label><label style="font-weight:500"><input style="width:auto;margin-right:8px" type="checkbox" name="require_movement_reason" <?=($settings['require_movement_reason']??'1')==='1'?'checked':''?>> Motif obligatoire pour chaque mouvement</label></div>
<div class="field"><label>Affichage des tableaux</label><label style="font-weight:500"><input style="width:auto;margin-right:8px" type="checkbox" name="compact_tables" <?=($settings['compact_tables']??'0')==='1'?'checked':''?>> Mode compact</label></div>
<div class="field"><label>Actions sensibles</label><label style="font-weight:500"><input style="width:auto;margin-right:8px" type="checkbox" name="confirm_sensitive_actions" <?=($settings['confirm_sensitive_actions']??'1')==='1'?'checked':''?>> Demander une confirmation avant actions sensibles</label></div>
</div>
...
<div class="card section"><h3>Matrice des autorisations</h3><div class="tablebox"><table><tr><th>Fonction</th><th>Gérant</th><th>Caissière</th><th>Cuisine</th></tr>
<tr><td>Consulter les produits</td><td>✓</td><td>✓</td><td>✓</td></tr>
<tr><td>Entrées / sorties</td><td>✓</td><td>✓</td><td>✓</td></tr>
<tr><td>Modifier un produit</td><td>✓</td><td>—</td><td>—</td></tr>
<tr><td>Consulter fournisseurs</td><td>✓</td><td>—</td><td>✓</td></tr>
<tr><td>Modifier fournisseurs</td><td>✓</td><td>—</td><td>—</td></tr>
<tr><td>Finance / Rapports</td><td>✓</td><td>—</td><td>—</td></tr>
<tr><td>Paramètres</td><td>✓</td><td>—</td><td>—</td></tr></table></div></div>
