...
function financeGraph($history,$symbol='€'){
 ...
 if(!$vals)return '';
 $n=count($vals);$min=min($vals);$max=max($vals);$range=max(1,$max-$min);
 $left=58;$right=750;$top=28;$bottom=210;$w=$right-$left;$h=$bottom-$top;
 $pts=[];foreach($vals as $i=>$v){$x=$n>1?$left+($i/($n-1))*$w:($left+$right)/2;$y=$bottom-(($v-$min)/$range)*$h;$pts[]=[$x,$y,$v];}
 $fmt=fn($v)=>number_format($v,0,',',' ').' '.$symbol;
 $html='<div style="overflow-x:auto"><svg class="finance-svg" viewBox="0 0 800 260" role="img" aria-label="Évolution de la valeur du stock">';
 for($i=0;$i<4;$i++){ $y=$top+($h/3)*$i;$val=$max-($range/3)*$i;$html.='<line class="finance-grid" x1="'.$left.'" y1="'.$y.'" x2="'.$right.'" y2="'.$y.'"/><text x="4" y="'.($y+4).'" font-size="11" fill="#6d737c">'.$fmt($val).'</text>'; }
 $points=implode(' ',array_map(fn($p)=>round($p[0],1).','.round($p[1],1),$pts));
 $html.='<polyline class="finance-line" points="'.$points.'"/>';
 foreach($pts as $i=>$p){$date=date('d/m H:i',strtotime($history[$i]['recorded_at']));$html.='<circle class="finance-dot" cx="'.$p[0].'" cy="'.$p[1].'" r="5"><title>'.$fmt($p[2]).' — '.$date.'</title></circle>';if($n<=12){$html.='<text x="'.$p[0].'" y="238" text-anchor="middle" font-size="10" fill="#777">'.$date.'</text>';}}
 $html.='</svg></div><div style="display:flex;gap:18px;flex-wrap:wrap;margin-top:8px;font-size:12px"><span><b>Départ :</b> '.$fmt($vals[0]).'</span><span><b>Actuel :</b> '.$fmt(end($vals)).'</span><span><b>Plus haut :</b> '.$fmt($max).'</span><span><b>Plus bas :</b> '.$fmt($min).'</span></div>';
 return $html;
}
